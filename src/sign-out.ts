...
export function safeDestination(value: unknown): { href: string; kind: DestinationKind } | null {
...
export function suiteSignOutDestination(sections: readonly { id: string; href: string }[]): string | null {
  const home = sections.find((section) => section.id === "home");
  if (!home || home.href === "#") return null;
  const destination = safeDestination(home.href);
  return destination ? destination.href : null;
}

export async function signOutOfSuite(url: string, destination: string): Promise<void> {
  const target = safeDestination(destination);
  if (!target) throw new Error("El destino de la salida no es válido.");
